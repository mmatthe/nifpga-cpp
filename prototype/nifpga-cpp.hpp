...
namespace nifpga {
  ...
  class fpga_exception : std::runtime_error  {
  public:
  ...
  void throwIfError(NiFpga_Status status, const char* message) {
    if (NiFpga_IsError(status)) {
      throw fpga_exception("FPGA error. Error code: " + boost::lexical_cast<std::string>(status) + "\n" + message);
    }
  }
...
#define readWriteRegister_instance(data_type, typestr)			\
  ...
  data_type readRegister(NiFpga_Session session, const Register<data_type, indicator_tag>& reg) {
    data_type result;
  ...
  template <class data_type>
  void writeRegister(NiFpga_Session session, const Register<data_type, control_tag>& reg, data_type value) {
  ...
  template <class T, class fifo_tag>
  class Fifo {
  public:
    typedef T data_type;
  ...
  template <class data_type, class fifo_tag>
  inline void configureFifo(NiFpga_Session session, const Fifo<data_type, fifo_tag>& fifo, size_t requested_depth) {
    throwIfError(NiFpga_ConfigureFifo(session, fifo.id, requested_depth), "NiFpga_ConfigureFifo");
  }

  template <class data_type, class fifo_tag>
  inline void startFifo(NiFpga_Session session, const Fifo<data_type, fifo_tag>& fifo) {
    throwIfError(NiFpga_StartFifo(session, fifo.id), "NiFpga_StartFifo");
  }

  template <class data_type, class fifo_tag>
  inline void stopFifo(NiFpga_Session session, const Fifo<data_type, fifo_tag>& fifo) {
...
#define readWriteFifo_impl_instance(data_type, typestr) \
  ...
  template <class data_type>
  void readFifo(NiFpga_Session session, Fifo<data_type, target_to_host_tag>& fifo, data_type* buffer_out, size_t number_of_elements, uint32_t timeout, size_t* elements_remaining) {
  ...
  template <class data_type>
  void writeFifo(NiFpga_Session session, Fifo<data_type, host_to_target_tag>& fifo, data_type* buffer_out, size_t number_of_elements, uint32_t timeout, size_t* elements_remaining) {
